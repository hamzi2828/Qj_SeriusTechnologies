                    ...
                    <?php
                        ...
                        if ( count ( $reports ) > 0 ) {
                            foreach ( $reports as $report ) {
                                ...
                                $status = get_report_verify_status ( $report -> id, 'hmis_culture' );
                                ?>
                                <tr class="odd gradeX">
                                    <td> <?php
                                            echo $counter ++ ?> </td>
                                    ...
                                    <td class="btn-group-xs">
                                        ...
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'print-culture-report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn dark btn-block" style="margin-bottom: 5px"
                                                   href="<?php
                                                       echo base_url ( '/invoices/culture-report?report-id=' . $report -> id . '&logo=true' ) ?>"
                                        ...
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'print-culture-report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn purple"
                                                   href="<?php
                                                       echo base_url ( '/invoices/culture-report?report-id=' . $report -> id ) ?>"
                                                   target="_blank">Print</a>
                                            <?php
                                            endif; ?>
                                        
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'edit-culture-report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn blue"
                                                   href="<?php
                                                       echo base_url ( '/culture/culture/edit?report_id=' . $report -> id ) ?>">Edit</a>
                                            <?php
                                            endif; ?>
                                        
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'verify_culture_report_button', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) && empty( $status ) ) : ?>
                                                <a type="button" class="btn green"
                                                   onclick="return confirm('Are you sure?')"
                                                   href="<?php
                                                       echo base_url ( '/culture/verify_report?report-id=' . $report -> id ) ?>">Verify</a>
                                            <?php
                                            endif; ?>
                                        
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'delete-culture-report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn red"
                                                   href="<?php
                                                       echo base_url ( '/culture/culture/delete/' . $report -> id ) ?>"
