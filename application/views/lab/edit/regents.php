<div class="tab-pane <?php if ( isset( $current_tab ) and $current_tab == 'regents' )
    echo 'active' ?>">
    <form role="form" method="post" autocomplete="off">
        <input id="csrf_token" type="hidden" name="<?php echo $this -> security -> get_csrf_token_name (); ?>"
               value="<?php echo $this -> security -> get_csrf_hash (); ?>">
        <input type="hidden" name="action" value="do_edit_test_regents">
                        ...
                        if ( count ( $lab_regents ) > 0 ) {
                            foreach ( $lab_regents as $lab_regent ) {
                                ...
                                <div class="form-group col-lg-12">
                                    <div class="col-md-6">
                                        <label>
                                            <?php if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'delete_lab_regents', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a href="<?php echo base_url ( '/lab/edit/?action=delete-regent&test-id=' . $test_id . '&regent-id=' . $lab_regent -> id ) ?>"
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control" name="usable_quantity[]"
                                               value="<?php echo $lab_regent -> usable_quantity ?>">
                                    </div>
                                </div>
                                <?php
                            }
        ...
        <div class="form-actions">
            <a href="javascript:void(0)" type="button" class="btn purple" style="display: inline"
               onclick="add_more_regents()">Add More</a>
