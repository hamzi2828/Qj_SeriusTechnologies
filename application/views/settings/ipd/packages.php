				...
				<table class="table table-striped table-bordered table-hover" id="sample_1">
					...
					if(count($packages) > 0) {
						...
						foreach ($packages as $package) {
						    $services = get_package_services($package -> id) -> services;
						    if($services)
							...
							<?php if(get_user_access(get_logged_in_user_id()) and in_array('edit_ipd_packages', explode(',', get_user_access(get_logged_in_user_id()) -> access))) : ?>
									<a type="button" class="btn blue" href="<?php echo base_url('/settings/edit-ipd-package/'.$package -> id.'?settings=ipd') ?>">Edit</a>
							<?php endif; ?>
                            <?php if(get_user_access(get_logged_in_user_id()) and in_array('delete_ipd_packages', explode(',', get_user_access(get_logged_in_user_id()) -> access))) : ?>
									<a type="button" class="btn red" href="<?php echo base_url('/settings/delete-ipd-package/'.$package -> id.'?settings=ipd') ?>" onclick="return confirm('Are you sure you want to delete?')">Delete</a>
							<?php endif; ?>
								</td>
							</tr>
							<?php
