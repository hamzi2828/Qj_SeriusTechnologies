    ...
    <div class="col-md-12">
        ...
        <div class="portlet box green">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i> Ultrasound Reports
                </div>
            </div>
                    ...
                    <?php
                        ...
                        if ( count ( $reports ) > 0 ) {
                            foreach ( $reports as $report ) {
                                ...
                                $status = get_report_verify_status ( $report -> id, 'hmis_ultrasound' );
                                ?>
                                <tr class="odd gradeX">
                                    <td> <?php
                                            echo $counter ++ ?> </td>
                                    ...
                                    <td class="btn-group-xs">
                                        ...
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'print_ultrasound_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn purple"
                                                   href="<?php
                                                       echo base_url ( '/invoices/ultrasound-report?report-id=' . $report -> id . '&logo=true' ) ?>"
                                        ...
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'print_ultrasound_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn dark btn-block" style="margin-bottom: 5px"
                                                   href="<?php
                                                       echo base_url ( '/invoices/ultrasound-report?report-id=' . $report -> id . '&logo=false' ) ?>"
                                                   target="_blank">Print</a>
                                            <?php
                                            endif; ?>
                                        
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'edit_ultrasound_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn blue" target="_blank"
                                                   href="<?php
                                                       echo base_url ( '/radiology/ultrasound/edit-ultrasound-report?report-id=' . $report -> id ) ?>">Edit</a>
                                            <?php
                                            endif; ?>
                                        
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'verify_ultrasound_report_button', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) && empty( $status ) ) : ?>
                                                <a type="button" class="btn green"
                                                   onclick="return confirm('Are you sure?')"
                                                   href="<?php
                                                       echo base_url ( '/radiology/x-ray/verify-ultrasound-report?report-id=' . $report -> id ) ?>">Verify</a>
                                            <?php
                                            endif; ?>
                                        
                                        <?php
                                            if ( get_user_access ( get_logged_in_user_id () ) and in_array ( 'delete_ultrasound_report', explode ( ',', get_user_access ( get_logged_in_user_id () ) -> access ) ) ) : ?>
                                                <a type="button" class="btn red"
                                                   href="<?php
                                                       echo base_url ( '/radiology/ultrasound/delete-ultrasound-report/' . $report -> id ) ?>"
